...
const NotFoundPage = () => {
  const defaultFlavorText = (
    <p className="fadeIn slowFadeIn">You enter an area of vast nothingness.</p>
  );
  const defaultOption1 = (
    <a className="inlineLink fadeIn slowFadeIn" href="/">
      <span style={{ marginRight: '1ch', display: 'inline-block' }}>a)</span>
      Flee this place
    </a>
  );
  const defaultOption2 = (
    <p
      onClick={() => tryRemembering()}
      className="inlineLink fadeIn slowFadeIn"
    >
      <span style={{ marginRight: '1ch', display: 'inline-block' }}>b)</span>
      Look around the space.
    </p>
  );

  const newFlavorText = (
    <p className="fadeIn slowFadeIn">
      You see a small screen displaying a three digit number.
    </p>
  );
  ...
  const flavor404 = (
    <p className="typewriter fadeIn slowFadeIn bordered">4 0 4</p>
  );
